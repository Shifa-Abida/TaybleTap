...
export default function AnalyticsDashboard() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#FFFDF8", fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside style={{
        width: 220, background: "white", borderRight: "1px solid #F0EDE8",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 20px 28px" }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 10, color: "#9CA3AF", fontWeight: 500 }}>Admin Panel</p>
          </div>
        </div>
            ...
            return (
              <button key={item.label} className="nav-btn"
                ...
                {item.label === "Live Orders" && (
                  <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, background: PRIMARY, color: "white", borderRadius: 10, padding: "1px 6px" }}>{activeCount}</span>
                )}
              </button>
            );
          })}
        ...
        <div style={{ margin: "0 14px", padding: "12px 14px", background: "rgba(255,107,53,0.06)", borderRadius: 12, border: "1px solid rgba(255,107,53,0.12)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 22 }}>🏪</span>
            <div>
              <p style={{ margin: 0, fontWeight: 600, fontSize: 12, color: "#1A1A1A" }}>{user?.restaurant_name || "My Restaurant"}</p>
              <p style={{ margin: 0, fontSize: 11, color: "#9CA3AF" }}>{user?.city || "Bengaluru"}</p>
            </div>
          </div>
        </div>
      </aside>
